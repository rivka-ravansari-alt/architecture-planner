...
_REQUIRED_KEYS = ["messages_per_month"]


def calculate_sns_quantities(
    ...
    messages = numeric_value(assumption_map, "messages_per_month")
    egress = optional_numeric(assumption_map, "data_egress_gb", 0)

    quantities: list[SkuQuantity] = [
        make_quantity(
            sku_key="requests",
            quantity=float(messages),
            unit="messages/month",
            formula="messages_per_month",
