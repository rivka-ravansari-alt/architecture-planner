"""Project-level AWS account free-tier pooling."""

from __future__ import annotations

from dataclasses import dataclass, field

from app.pricing.aws.allowances import (
    POOL_FREE_TIER_ALLOWANCES,
    allowance_key_for_sku,
    pool_id_for_service,
)
from app.pricing.aws.sku_quantities._helpers import assumptions_by_key, string_value
...
def _is_free_tier_eligible(
    ...
    if service == "ECS Fargate":
        return False, "ECS Fargate has no always-free tier allowances."
    ...
    if service == "RDS":
        ...
        if "instance_class" in assumption_map:
            instance_class = string_value(assumption_map, "instance_class").strip().casefold()
        else:
            instance_class = instance_class.casefold()
        if "micro" not in instance_class and "t2" not in instance_class and "t3" not in instance_class:
            return (
                False,
                f"Free tier applies to db.t2/db.t3.micro only (instance_class={instance_class!r}).",
            )
        return True, None

    if service in {"Lambda", "SQS", "API Gateway", "SNS", "SES", "CloudWatch", "CloudWatch Logs", "CloudWatch Dashboards", "CloudWatch Alarms", "SSM Parameter Store", "X-Ray"}:
        return True, None
                ...
                if deduction > 0:
                    included_usage.append(
                        IncludedUsage(
                            ...
                            unit=quantity.unit,
                            description=(
                                f"AWS Free Tier grant applied to {quantity.sku_key!r}."
                            ),
                            source="free_tier",
                        )
                    )
