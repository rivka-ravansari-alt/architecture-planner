...
class AwsMeterUnitScaler:
    ...
    _TRACE_PATTERN = re.compile(r"\btrace", re.IGNORECASE)
    ...
    _LCU_HOUR_PATTERN = re.compile(r"lcu[\s-]*hr", re.IGNORECASE)
    ...
    def scale(
        ...
        if self._TRACE_PATTERN.search(meter) and "trace" in qty_unit:
            if self._PER_1M_PATTERN.search(meter):
                return quantity / 1_000_000, f"{quantity:,.0f} traces / 1,000,000"
            return quantity, f"{quantity:,.0f} traces (direct)"
        ...
        # GB-seconds must be checked before generic GB / GB-month patterns.
        ...
        if self._LCU_HOUR_PATTERN.search(meter) and "lcu" in qty_unit and "hour" in qty_unit:
            return quantity, f"{quantity:,.0f} LCU-hours (direct)"
