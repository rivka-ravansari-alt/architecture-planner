"""Cloud Run SKU quantity formulas."""
...
def calculate_cloud_run_quantities(
    ...
    min_instances = int(numeric_value(assumption_map, "min_instances"))
    ...
    warnings: list[str] = []
    if min_instances > 0:
        baseline_vcpu_hours = min_instances * cpu * MONTHLY_HOURS
        baseline_memory_gb_hours = min_instances * memory_gb * MONTHLY_HOURS
        ...
        if baseline_vcpu_hours > activity_vcpu_hours:
            warnings.append("min_instances baseline exceeds request-driven activity for vcpu_hours.")
    else:
    ...
    quantities: list[SkuQuantity] = [
        make_quantity(
            ...
            formula="max(requests * duration * cpu / 3600, min_instances * cpu * hours_per_month)",
        ...
        make_quantity(
            ...
            formula="max(requests * duration * memory_gb / 3600, min_instances * memory_gb * hours_per_month)",
            assumption_map=assumption_map,
            input_keys=compute_input_keys,
            warnings=warnings,
        ),
