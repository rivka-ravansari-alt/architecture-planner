"""Cloud Pub/Sub SKU quantity formulas."""
...
_REQUIRED_KEYS = ["message_operations"]


def calculate_cloud_pubsub_quantities(
    ...
    message_operations = numeric_value(assumption_map, "message_operations")
    data_egress_gb = optional_numeric(assumption_map, "data_egress_gb", 0)

    quantities: list[SkuQuantity] = [
        make_quantity(
            sku_key="requests",
            quantity=float(message_operations),
            unit="operations/month",
            formula="message_operations",
