"""Multi-component GCP SKU quantity orchestration with shared free-tier pools."""
...
def calculate_project_gcp_sku_quantities(
    ...
    for component in sorted(components, key=lambda item: item.order):
        model = get_gcp_pricing_model(component.cloud_service)
        if model is None:
            results.append(
                ComponentSkuResult(
                    ...
                    missing=[],
                    ready=False,
                    warnings=[f"Unknown GCP service {component.cloud_service!r}."],
                )
            )
            continue

        raw = calculate_gcp_sku_quantities(model, component.resolved)
