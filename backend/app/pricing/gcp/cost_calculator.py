"""Calculate monthly GCP costs from billable SKU quantities and catalog prices."""
...
class GcpCostCalculator:
    ...
    def __init__(
        self,
        catalog_lookup: GcpCatalogLookup,
        role_resolver: GcpSkuRoleResolver,
        meter_scaler: GcpMeterUnitScaler,
    ) -> None:
    ...
    def calculate_component(
                ...
                if price is None:
                    missing_prices.append(
                        MissingCatalogPrice(
                            ...
                            sku_key=quantity.sku_key,
                            catalog_role=role,
                            reason=f"Catalog role {role!r} not found in gcp_catalog.",
                        )
                    )
                    continue
        ...
        missing_from_calculation = model_sku_keys - {q.sku_key for q in adjusted.quantities}
