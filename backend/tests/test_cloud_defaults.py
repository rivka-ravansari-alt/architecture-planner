...
def test_applies_selected_catalog_cloud_mapping_by_type(ai_validator, cloud_defaults_service):
    payload = json.loads(VALID_AI_RESPONSE_JSON)
    payload["components"][0]["cloud_mappings"] = {"aws": "S3"}
    validated = ai_validator.validate(json.dumps(payload))
    component_type = validated["components"][0]["type"]
    expected = cloud_defaults_service.default_cloud_mapping_for_type(component_type)
    assert validated["components"][0]["cloud_mappings"] == expected


def test_user_type_gets_not_applicable(cloud_defaults_service):
    defaults = cloud_defaults_service.default_cloud_mapping_for_type("user")
    assert all(option == "N/A" for option in defaults.values())
