"""Generate AWS pricing coverage report."""
...
from app.pricing.aws.coverage import build_aws_pricing_coverage_report
...
def format_text_report(report) -> str:
    lines = [
        "AWS PRICING COVERAGE REPORT",
        "=" * 100,
        "",
        f"{'Service':<28} {'Types':<18} {'Model':>5} {'SKU':>5} {'Roles':>5} "
    ...
    for item in report.services:
        ...
        if len(item.component_types) > 2:
            types += "…"
        lines.append(
            f"{item.service_name[:28]:<28} {types[:18]:<18} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AWS pricing coverage report.")
